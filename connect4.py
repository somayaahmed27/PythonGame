...
def draw_board(board):
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            pygame.draw.rect(screen, BLUE, (c*SQUARSIZE, r *
                             SQUARSIZE+SQUARSIZE, SQUARSIZE, SQUARSIZE))
            pygame.draw.circle(screen, BLACK, (int(
                c*SQUARSIZE+SQUARSIZE/2), int(r*SQUARSIZE+SQUARSIZE+SQUARSIZE/2)), RADIUS)
    for c in range(COLUMN_COUNT):
        for r in range(ROW_COUNT):
            if board[r][c] == PLAYER_PIECE:
                pygame.draw.circle(screen, RED, (int(
                    c*SQUARSIZE+SQUARSIZE/2), height - int(r*SQUARSIZE+SQUARSIZE/2)), RADIUS)
...
while not game_over:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if turn == PLAYER:
                pygame.draw.circle(
                    screen, RED, (posx, int(SQUARSIZE/2)), RADIUS)
